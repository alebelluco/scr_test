...
if not st.toggle('Nessun nuovo dato, visualizza storico'):
    ...
    #db_fogli = db_fogli[db_fogli.VEL_TIRATURA_PREVISTA.astype(str) != 'nan']
    #db_fogli = db_fogli[db_fogli.VEL_TIRATURA_PREVISTA != 0]
    ...
    with tab1:
        ...
        with c1_st:
            db_ore_stam
            st.metric('FOGLI ORA UOMO GIORNATA | indiretti INCLUSI', value='{:0.0f}'.format(db_ore_stam['QTA_PRODOTTA'].sum()/db_ore_stam['Ore_totali'].sum()), border=True)
        with c2_st:
        ...
        with c1_fs:
            db_ore_fus
            st.metric('FOGLI ORA UOMO GIORNATA | indiretti INCLUSI', value='{:0.0f}'.format(db_ore_fus['QTA_PRODOTTA'].sum()/db_ore_fus['Ore_totali'].sum()), border=True)
        ...
        with c1_in:
            db_ore_inco
            st.metric('ORE_uomo / ORE_std GIORNATA | indiretti INCLUSI', value='{:0.2f}'.format(db_ore_fus['Ore_totali'].sum()/db_ore_fus['ton'].sum()), border=True)

        with c2_in:
            #st.metric('PEZZI ORA UOMO T1', value=db_ore_inco['F/mh'].iloc[0], border=True)
            st.metric('ORE_uomo / ORE_std T1', value='{:0.2f}'.format(db_ore_fus['Presenza'].iloc[0]/db_ore_fus['ton'].iloc[0]), border=True)
        with c3_in:
            #st.metric('PEZZI ORA UOMO T2', value=db_ore_inco['F/mh'].iloc[1], border=True)
            st.metric('ORE_uomo / ORE_std T2', value='{:0.2f}'.format(db_ore_fus['Presenza'].iloc[1]/db_ore_fus['ton'].iloc[1]), border=True)
